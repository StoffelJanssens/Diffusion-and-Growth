...
def concentration_array_r_bound_large(array_nodes, sigma_bound):
    ...
    if r_omega > 100:
        array_sigma = 1 + (sigma_bound - 1) * np.exp(r_array - r_omega)
    else:
...
def sim_vs_exp(params, args):
    d_alpha, sigma_omega_scaled = params

    # calling the args and scale with diffusion length
    node_array, node_array_end, g_e, dist_max, remesh_node_num = args

    node_array = node_array / d_alpha
    g_e = g_e / d_alpha
    dist_max = dist_max / d_alpha
    ...
    # --- Obtaining the metric for minimization ---
    node_array = remeshing(node_array, remesh_node_num)
    # Calculate the sum of the squared residuals
    dist = np.sum(np.sum((node_array - node_array_end) ** 2, axis=1))
...
def simulation(params, args):
    ...
    while not growth_completed:
        ...
        # print('Progress 1 = ', g * 100 / g_e, '%')

    return node_array * dif_length


# --- Input ---
growth_param_after = 1.0  # g_e
initial_guess_d_alpha = 0.5  # initial guess d_\alpha
initial_guess_sigma_o_sigma_a = 2.5  # initial guess \sigma_omega/sigma_\alpha
...
initial_guesses = [initial_guess_d_alpha,
                   initial_guess_sigma_o_sigma_a]

arguments = [node_array_before,
             node_array_after,
             growth_param_after,
             edge_len_max,
             node_num_remesh]

fitting_result = minimize(sim_vs_exp, initial_guesses, arguments, method='Nelder-Mead')

outcome = fitting_result.success
d_alpha_fit = fitting_result.x[0]
sigma_omega_scaled_fit = fitting_result.x[1]

print('Minimize successful: ', outcome)
print('diffusion length: ', d_alpha_fit)
print('scaled boundary concentration: ', sigma_omega_scaled_fit)

# --- Plot arrays ---
# Given arrays
plt.plot(node_array_before[:, 0], node_array_before[:, 1], color='#2c7fb8')
plt.plot(node_array_after[:, 0], node_array_after[:, 1], color='#7fcdbb')

# Fitted array
given_val_param_list = [d_alpha_fit,
                        sigma_omega_scaled_fit]

arg_list_given = [node_array_before,
                  growth_param_after,
                  edge_len_max]

node_array_after_fit = simulation(given_val_param_list, arg_list_given)

plt.plot(node_array_after_fit[:, 0], node_array_after_fit[:, 1], color='black', linestyle=(0, (3, 3)))
