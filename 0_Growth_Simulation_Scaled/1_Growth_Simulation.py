...
def concentration_array(array_nodes, cap_c, concent_beta, diff_len_beta, cap_d):
    ...
    if r_omega > 100:
        concent_bound = (concent_beta + d_beta_cap_d) / (cap_c + d_beta_cap_d)
        array_concent = 1 + (concent_bound - 1) * np.exp(r_array - r_omega)
    else:
        ...
        array_concent = 1 + 2 * (concent_bound - 1) * np.exp(-r_omega) * np.cosh(r_array)
...
def simulation(array_nodes, arguments):
    green_lines_num, array_length_min_incl, cap_c, concent_beta, diff_len_beta, cap_d, max_dist = arguments

    min_dist = max_dist * 0.49
    ...
    while not growth_completed:
        ...
        array_nodes = topological_merging(array_nodes, min_dist, array_length_min_incl)

        # Remesh by removing neighboring nodes that are too close.
        array_nodes = lower_bound_distance(array_nodes, min_dist)

        # Adding nodes.
        array_nodes = upper_bound_distance(array_nodes, max_dist)
        ...
        if sigma_first <= sigma_last:
            delta_g = min_dist / sigma_last
        else:
            delta_g = min_dist / sigma_first
...
# node_array = np.array([[0, 0], [200, 0]], dtype=float)
